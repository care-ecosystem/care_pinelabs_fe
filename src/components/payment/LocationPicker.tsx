...
import { LocationRead, LocationTypeIcons } from "@/types/location";
...
export const LocationPicker: FC<LocationPickerProps> = ({
  ...
  const {
    ...
    queryKey: ["pinelabs_locations", facilityId, currentParent, searchQuery],
    queryFn: () =>
  ...
  useEffect(() => {
    if (open && value?.parent) {
      ...
      let current: LocationRead | null | undefined = value.parent;
  ...
  const getDisplayValue = () => {
    ...
    const Icon = LocationTypeIcons[value.form];
